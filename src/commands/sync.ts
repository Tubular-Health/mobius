/**
 * Sync command - Push pending local changes to Linear/Jira via SDK
 *
 * Reads pending-updates.json files from local context and executes the
 * appropriate SDK calls to sync changes to the backend.
 */
...
/**
 * Sync pending updates to Linear/Jira
 *
 * @param parentId - Optional parent issue ID to sync (syncs all if not specified with --all)
 * @param options - Command options
 */
export async function sync(parentId: string | undefined, options: SyncOptions): Promise<void> {
  const paths = resolvePaths();
  const config = readConfig(paths.configPath);
  const backend = options.backend ?? config.backend;

  // Determine which issues to sync
  const issuesToSync = getIssuesToSync(parentId, options.all);

  if (issuesToSync.length === 0) {
    if (parentId) {
      console.error(chalk.red(`Error: No pending updates found for ${parentId}`));
      ...
      console.error(chalk.gray('Specify an issue ID or use --all to sync all pending updates'));
    }
    process.exit(1);
  }
  ...
  if (parentId) {
    ...
    if (!pattern.test(parentId)) {
      console.error(chalk.red(`Error: Invalid issue ID format for ${backend}: ${parentId}`));
  ...
  for (const issueId of issuesToSync) {
  ...
  if (totalPending === 0) {
    console.log(chalk.green('No pending updates to sync'));
    return;
  }

  // Dry run mode - show what would be synced
  if (options.dryRun) {
    console.log(chalk.bold('\nDry run - pending changes to sync:\n'));
    displayPendingChanges(allUpdates, backend);
    console.log(chalk.gray(`\nTotal: ${totalPending} update(s) across ${issuesToSync.length} issue(s)`));
  ...
  const syncSpinner = ora({
    text: `Syncing ${totalPending} update(s) to ${backend}...`,
    color: 'blue',
  }).start();
  ...
  for (const { parentId: issueParentId, update } of allUpdates) {
    const result = await syncUpdate(update, backend);
    results.push(result);
    ...
    logSyncResult(issueParentId, result);
  }

  if (failureCount === 0) {
    syncSpinner.succeed(`Successfully synced ${successCount} update(s)`);
  } else if (successCount === 0) {
    syncSpinner.fail(`Failed to sync all ${failureCount} update(s)`);
  } else {
    syncSpinner.warn(`Synced ${successCount} update(s), ${failureCount} failed`);
  }

  // Display summary
  console.log('');
  displaySyncSummary(results);

  if (failureCount > 0) {
    console.log(chalk.gray('\nFailed updates remain in pending-updates.json'));
    console.log(chalk.gray('Fix the issues and run sync again'));
    process.exit(1);
  }
}
 ...
 */
function getIssuesToSync(parentId: string | undefined, all: boolean | undefined): string[] {
...
/**
 * Sync a single update to the backend
 */
async function syncUpdate(update: PendingUpdate, backend: Backend): Promise<SyncResult> {
  const baseResult = {
    updateId: update.id,
    type: update.type,
  ...
  try {
    switch (update.type) {
      case 'status_change':
        return await syncStatusChange(update, backend, baseResult);
      case 'add_comment':
        return await syncAddComment(update, backend, baseResult);
      case 'create_subtask':
        return await syncCreateSubtask(update, backend, baseResult);
      case 'update_description':
...
/**
 * Sync a status change
 */
async function syncStatusChange(
  update: Extract<PendingUpdate, { type: 'status_change' }>,
  backend: Backend,
  baseResult: Omit<SyncResult, 'success' | 'error'>
): Promise<SyncResult> {
  if (backend === 'linear') {
...
/**
 * Sync a create subtask operation
 */
async function syncCreateSubtask(
...
/**
 * Log sync result to sync-log.json
 */
function logSyncResult(parentId: string, result: SyncResult): void {
 ...
 */
function displaySyncSummary(results: SyncResult[]): void {
