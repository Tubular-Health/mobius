...
pub fn write_config(config_path: &str, config: &LoopConfig) -> Result<(), ConfigError> {
    ...
    let content = format!(
        "# Mobius Configuration\n\
         ...
         #   MOBIUS_RUNTIME, MOBIUS_BACKEND, MOBIUS_DELAY_SECONDS, etc.\n\
         \n\
         {yaml}"
    );
...
"#;
    ...
    fn test_read_config_with_runtime_override() {
        ...
        env::set_var("MOBIUS_RUNTIME", "opencode");
        let config = read_config_with_env(config_path.to_str().unwrap()).unwrap();
        assert_eq!(config.runtime, AgentRuntime::Opencode);
        env::remove_var("MOBIUS_RUNTIME");
    }

    #[test]
    fn test_read_config_parses_runtime_from_yaml() {
        let tmp = tempfile::tempdir().unwrap();
        let config_path = tmp.path().join("config.yaml");
    ...
    fn test_write_config_roundtrip() {
        ...
        assert_eq!(
            reloaded.execution.delay_seconds,
    ...
    fn test_validate_config_invalid_parallel_agents() {
        ...
        assert!(result
            .errors
            .iter()
            .any(|e| e.contains("max_parallel_agents")));
    ...
    fn test_validate_config_verification_threshold() {
        ...
        assert!(result
            .errors
            .iter()
            .any(|e| e.contains("coverage_threshold")));
    }
